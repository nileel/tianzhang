# Round 2: Adjusted reaction coefficients + balanced builds + comprehensive testing
...
# Build definitions
$builds = @(
    # Name, innate(根骨,魂魄,神识,资质,气运), style, weights
    ...
    @{Name="均衡型A";  Innate=@{根骨=25;魂魄=25;神识=25;资质=20;气运=20}; Style="physical"; W=$wBal},
    @{Name="均衡型B";  Innate=@{根骨=25;魂魄=25;神识=25;资质=20;气运=20}; Style="spiritual"; W=$wBal},
    @{Name="肉盾型";   Innate=@{根骨=43;魂魄=12;神识=22;资质=18;气运=20}; Style="physical"; W=$wPhys},
    @{Name="刺客型";   Innate=@{根骨=18;魂魄=20;神识=40;资质=19;气运=18}; Style="physical"; W=@{根骨=0.6;魂魄=0.6;神识=1.0;资质=0.6;气运=0.7}},
    @{Name="灵修型";   Innate=@{根骨=15;魂魄=43;神识=18;资质=20;气运=19}; Style="spiritual"; W=$wSpir}
)

# Build all characters (筑基 后期, 上品, 中品灵根)
$chars = @()
foreach ($b in $builds) {
    $chars += BuildChar $b.Name $b.Innate "筑基" 2 "上品" $b.W "中品" $b.Style
}

Write-Host "============================="
Write-Host "  角色面板 (筑基后期/上品/中品)"
Write-Host "============================="
foreach ($c in $chars) {
    Write-Host "`n--- $($c.Name) ---"
    Write-Host "  先天: 骨$($c.根骨) 魂$($c.魂魄) 神$($c.神识) 资$($c.资质) 运$($c.气运)"
    Write-Host "  战斗: HP$($c.HP) MP$($c.MP) 肉攻$($c.肉攻)/神攻$($c.神攻) 肉防$($c.肉防)/神防$($c.神防) 反应$($c.反应) 移$($c.移力) 识$($c.神识)"
    Write-Host "  二级: 生命恢复$($c.生命恢复)/t 格挡$($c.格挡率)% 物抗$($c.物抗率)% 暴击$($c.暴击率)% 暴伤$($c.暴击伤害)% 命中$($c.命中率)% 闪避$($c.闪避率)%"
}

Write-Host "`n============================="
Write-Host "  战斗模拟矩阵 (每对1000次)"
Write-Host "============================="

$SIM = 1000
$names = $chars | ForEach-Object { $_.Name }

# Header
$header = "           "
foreach ($n in $names) { $header += "$($n.PadLeft(10))" }
Write-Host $header

for ($i = 0; $i -lt $chars.Count; $i++) {
    $row = "$($names[$i].PadRight(10))"
    for ($j = 0; $j -lt $chars.Count; $j++) {
        if ($i -eq $j) { $row += "     ---  "; continue }
        $wA=0; $wB=0
        for ($k=0; $k -lt $SIM; $k++) {
            $r = SimOne $chars[$i] $chars[$j]
            if ($r.Winner -eq "A") { $wA++ } elseif ($r.Winner -eq "B") { $wB++ }
        }
        $pct = [Math]::Round($wA/$SIM*100)
        $row += "  $($pct.ToString().PadLeft(3))%   "
    }
    Write-Host $row
}

Write-Host "`n(数字 = 行角色胜率)"
...
# Key matchups
$pairs = @(
    @{A=0;B=1;Label="物理专精 vs 法术专精"},
    @{A=2;B=3;Label="均衡物理 vs 均衡法术"},
    @{A=0;B=2;Label="物理专精 vs 均衡物理"},
    @{A=1;B=3;Label="法术专精 vs 均衡法术"},
    @{A=4;B=1;Label="肉盾 vs 法术专精"},
    @{A=5;B=1;Label="刺客 vs 法术专精"}
)
...
foreach ($p in $pairs) {
    ...
    Write-Host "`n[$($p.Label)]"
    Write-Host "  伤害: $($a.Name)->$dAB vs $($b.Name)->$dBA"
    Write-Host "  HP: $($a.Name)=$($a.HP) vs $($b.Name)=$($b.HP)"
    Write-Host "  反应: $($a.Name)=$($a.反应) vs $($b.Name)=$($b.反应)"
    Write-Host "  胜率: $([Math]::Round($wA/$SIM*100,1))% - $([Math]::Round($wB/$SIM*100,1))%"
    Write-Host "  平均ticks: $([Math]::Round($tT/$SIM,1))  行动: $([Math]::Round($tAA/$SIM,1))/$([Math]::Round($tAB/$SIM,1))"
}
