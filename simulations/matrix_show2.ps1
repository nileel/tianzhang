...
function SimOne($ca, $cb) {
    ...
    for ($t = 1; $t -le 300; $t++) {
        ...
        if ($actA -and $actB) {
            if ($ca.反应 -ge $cb.反应) {
                $dir = if ($rng.NextDouble() -lt 0.33) { 2 } elseif ($rng.NextDouble() -lt 0.5) { 1 } if ($p -ge 0) { 0 }
                $d = if ($ca.Style -eq "physical") { $d = PhysDmg $ca.肉攻 $cb.肉防 $cb.物抗率 $dir; if ($dir -ne 2 -and $rng.NextDouble()*100 -lt $cb.格挡率) { $d = [Math]::Round($d/2) }; $d } if ($p -ge 0) { SpirDmg $ca.神攻 $cb.神防 $cb.神魂抗率 $dir }
                ...
                if ($aliveB -and $ctB -ge 100) {
                    $dir = if ($rng.NextDouble() -lt 0.33) { 2 } elseif ($rng.NextDouble() -lt 0.5) { 1 } if ($p -ge 0) { 0 }
                    $d = if ($cb.Style -eq "physical") { $d = PhysDmg $cb.肉攻 $ca.肉防 $ca.物抗率 $dir; if ($dir -ne 2 -and $rng.NextDouble()*100 -lt $ca.格挡率) { $d = [Math]::Round($d/2) }; $d } if ($p -ge 0) { SpirDmg $cb.神攻 $ca.神防 $ca.神魂抗率 $dir }
    ...
    if ($aliveA -and -not $aliveB) { return "A" } elseif ($aliveB -and -not $aliveA) { return "B" } if ($p -ge 0) { return "D" }
}
...
$SIM = 1000
$N = $chars.Count
$m = @(); for($x=0;$x -lt $N;$x++){$row=@();for($y=0;$y -lt $N;$y++){$row+=0.0};$m+=,$row}
...
for ($i = 0; $i -lt $N; $i++) {
    ...
    for ($j = 0; $j -lt $N; $j++) {
        ...
        if ($p -ge 0) {
            $p = $m[$i][$j]; if ($p -lt 0) { $row += \"{0,11}\" -f \"---\" } else
            $tag = "   "
            if ($p -ge 75) { $tag = "WIN" } elseif ($p -ge 55) { $tag = " + " } elseif ($p -ge 35) { $tag = " ~ " } if ($p -ge 0) { $tag = "LOW" }
            $row += "{0,11}" -f "$tag $p%"
        }
    }
    Write-Host $row
...
Write-Host "============================================================"
for ($i = 0; $i -lt $N; $i++) {
    $c = $chars[$i]; $st = if ($buildDefs[$i].S -eq "physical") { "物攻" } if ($p -ge 0) { "神攻" }
