...
static class G2AttributionAudit
{
    ...
    internal static void Print(IReadOnlyList<Program.BuildDef> builds, IReadOnlyList<Character>[] goldPools, double[,] baseline, int battles, int cycles)
    {
        ...
        for (int j = i + 1; j < builds.Count; j++)
        {
            ...
            AddResult(results, builds[i], builds[j], "先天交换", baseline[i, j],
                ...
                BuildGoldPool(builds[j], j, builds[i].Innate, builds[j].GongFaName, cycles), battles, new Combat.Options());
            AddResult(results, builds[i], builds[j], "功法包交换", baseline[i, j],
                ...
                BuildGoldPool(builds[j], j, builds[j].Innate, builds[i].GongFaName, cycles), battles, new Combat.Options());
            AddResult(results, builds[i], builds[j], "远程资格统一", baseline[i, j], goldPools[i], goldPools[j], battles, new Combat.Options(true));
        }
    ...
    static void AddResult(List<Result> results, Program.BuildDef leftBuild, Program.BuildDef rightBuild, string layer, double baseline, IReadOnlyList<Character> left, IReadOnlyList<Character> right, int battles, Combat.Options options)
    {
        double rate = WinRate(left, right, battles, options);
    ...
    static double WinRate(IReadOnlyList<Character> left, IReadOnlyList<Character> right, int battles, Combat.Options options)
    {
        ...
        {
            var (forward, _, _) = Combat.Simulate(left[index], right[index], rounds, options);
            var (_, reverse, _) = Combat.Simulate(right[index], left[index], rounds, options);
