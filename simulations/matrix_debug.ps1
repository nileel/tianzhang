...
# DEBUG
$testA = $chars[0]; $testB = $chars[1]
$testWins = 0
for ($dk = 0; $dk -lt 100; $dk++) { $r = SimOne $testA $testB; if ($r -eq "A") { $testWins++ } }
Write-Host ("DEBUG: SimOne returns type=" + (SimOne $testA $testB).GetType().Name + " testWins=$testWins/100")
...
Write-Host "`n================================================================"
Write-Host "  筑基后期 战斗胜率矩阵 (row attacks col, $SIM battles/pair)"
