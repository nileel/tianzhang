...
# Init 2D array properly
$m = New-Object 'double[,]' $N, $N

Write-Host "Simulating $($N)x$($N) matrix ($SIM battles/pair)..."
for ($i = 0; $i -lt $N; $i++) {
    for ($j = 0; $j -lt $N; $j++) {
        if ($i -eq $j) { $m[$i,$j] = -1; continue }
        $wins = 0
        # Symmetric: average of both roles
        $winsAsA = 0; $winsAsB = 0
        $half = [Math]::Floor($SIM / 2)
        for ($k = 0; $k -lt $half; $k++) {
            if ((SimOne $chars[$i] $chars[$j]).Winner -eq "A") { $winsAsA++ }
            if ((SimOne $chars[$j] $chars[$i]).Winner -eq "B") { $winsAsA++ }
        }
        $m[$i,$j] = [Math]::Round($winsAsA / $SIM * 100, 1)
    }
}

Write-Host "`n================================================================"
Write-Host "  筑基后期 战斗胜率矩阵 (row vs col, symmetric avg, $SIM battles/pair)"
Write-Host "  条件: 上品功法/中品灵根/纯普攻/无道基/无术法"
Write-Host "================================================================"
Write-Host ""

# Header
$hdr = "{0,-10}" -f "ATK\\DEF"
for ($j = 0; $j -lt $N; $j++) { $hdr += "{0,12}" -f $buildDefs[$j].N }
Write-Host $hdr
Write-Host ("-" * (10 + 12*$N))

for ($i = 0; $i -lt $N; $i++) {
    $rowStr = "{0,-10}" -f $buildDefs[$i].N
    for ($j = 0; $j -lt $N; $j++) {
        $p = $m[$i,$j]
        if ($p -lt 0) {
            $rowStr += "{0,12}" -f "---"
        } else {
            if ($p -ge 80) { $tag = "CRUSH" }
            elseif ($p -ge 60) { $tag = "FAVOR" }
            elseif ($p -ge 40) { $tag = "EVEN " }
            else { $tag = "WEAK " }
            $rowStr += "{0,12}" -f "$tag $p%"
        }
    }
...
Write-Host ""
Write-Host "  CRUSH=80%+(碾压)  FAVOR=60-80%(优势)  EVEN=40-60%(均势)  WEAK=<40%(劣势)"
Write-Host ""
...
Write-Host "================================================================"
for ($i = 0; $i -lt $N; $i++) {
    $c = $chars[$i]; $st = if ($buildDefs[$i].S -eq "physical") { "[物]" } else { "[神]" }
    Write-Host ("  {0} {1} HP={2} MP={3} 肉攻={4} 神攻={5} 肉防={6} 神防={7} 反应={8} 格挡={9}% 物抗={10}% 魂抗={11}%" -f $buildDefs[$i].N, $st, $c.HP, $c.MP, $c.肉攻, $c.神攻, $c.肉防, $c.神防, $c.反应, $c.格挡率, $c.物抗率, $c.神魂抗率)
}
...
Write-Host "  解读"
Write-Host "================================================================"
$pPhySpr = $m[0,1]; $pSprPhy = $m[1,0]
$pBal1 = $m[2,3]; $pBal2 = $m[3,2]
$pPhyTk = $m[0,4]; $pTkPhy = $m[4,0]
$pSprLx = $m[1,5]; $pLxSpr = $m[5,1]

Write-Host ""
Write-Host ("  物理专精 vs 法术专精 = " + $pPhySpr + "% / " + $pSprPhy + "%")
Write-Host "    -> 根骨->3项一级属性(HP+肉攻+肉防) vs 魂魄->2项(MP+神攻)"
Write-Host "       物理有先天优势，但魂魄攻系数(6.5)补偿了部分差距"
Write-Host "       带术法后神攻型MP充裕的优势才能体现"
Write-Host ""

Write-Host ("  均衡物理 vs 均衡法术 = " + $pBal1 + "% / " + $pBal2 + "%")
$balComment = "有小幅倾斜"
if ($pBal1 -gt 42 -and $pBal1 -lt 58) { $balComment = "基本五五开" }
Write-Host ("    -> 属性完全对称，" + $balComment)
Write-Host ""

Write-Host ("  物理专精 vs 肉盾型     = " + $pPhyTk + "% / " + $pTkPhy + "%")
$tankComment = "克制不够明显"
if ($pPhyTk -lt 35) { $tankComment = "克制关系成立" }
Write-Host ("    -> " + $tankComment + "：肉盾更高HP+更高肉防+更高物抗")
Write-Host ""

Write-Host ("  法术专精 vs 灵修型     = " + $pSprLx + "% / " + $pLxSpr + "%")
$lxComment = "一侧明显占优"
if ($pSprLx -gt 40 -and $pSprLx -lt 65) { $lxComment = "较为均势" }
Write-Host ("    -> " + $lxComment + "：灵修是法术的肉盾版，魂抗更高")
