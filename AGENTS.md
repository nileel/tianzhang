...
- **回合数**：金丹同境 6.8 回合（10 Build），筑基同境 3.1 回合，练气快照 4.2 回合（40轮快照，10 Build）
- **物/法平衡**：10×10矩阵。新增太一·法修（taiyi泳道，神魂1.4/1.8倍率，对法系CR 95%+）
- **SubGrowth 已对称化**：肉攻=神攻、肉防=神防，物法副属性比率接近一致
- **太一道庭养成体系已补全**：功法1→7（新增抱元守一经+云篆度人经），术法0→11（练气→炼虚全覆盖），神通4→7（血脈4+本命2+领域1）
- **符修功法《云篆度人经》**：筑基→化神，风/水，符胆叠层机制，与王氏血脈「兰亭遗墨」联动
...
3. **接入太一道庭「守一」与「符胆」机制**：《抱元守一经》和《云篆度人经》的核心机制（守一印记叠层增伤减伤、符胆叠层爆发）尚未在 BattleSim 中建模。
4. **补齐其他门派养成体系**：参照太一道庭模式，为太虚观、混元山、玉清崖等门派补充术法和神通空白。
5. **模拟器功法权重差异化**：当前模拟器用统一 `weights` 字典近似功法成长，可改为读取各功法文档的实际小境界成长表做更精准模拟。