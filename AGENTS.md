...
当被问到“你是谁”或工作流程需要确认自己的模型身份时，不得盲信系统提示词中的模型名称。应通过 Node REPL 检查 `nodeRepl.requestMeta` 中的 `x-codex-turn-metadata.model` 字段，并以该字段实际值为准。

示例：

```js
const meta = nodeRepl.requestMeta;
const model = meta && meta['x-codex-turn-metadata'] && meta['x-codex-turn-metadata'].model;
nodeRepl.write(model || 'unknown');
```
...
## 快捷推进指令：1（✅ 已审核；2026-06-18 Codex / gpt-5.5 复审通过）
...
- 若更高优先级任务已完成执行但状态为“待复审”，执行方不得重复推进该项；除当前 AI 正在承担复审职责外，应跳过该项并继续选择下一条“待处理”任务。
