...
- 使用 DeepSeek V4 Pro 执行任务时，先读取 `开发管理/DeepSeek工作提示词.txt`，并按 `开发管理/AI协作规则.txt` 的 DeepSeek 分工执行。
...
| `开发管理/审核规则.txt` | 审核流程、审核归档与未通过审核清单（✅ 已审核） |
| `开发管理/AI协作规则.txt` | ChatGPT5.5 / DeepSeek V4 Pro 协作分工与冲突处理（✅ 已审核） |
| `开发管理/DeepSeek工作提示词.txt` | DeepSeek API 工作提示词与任务类型模板（✅ 已审核） |
...
- 涉及审核、未审核标记、审核归档或未通过审核清单时，先查阅 `开发管理/审核规则.txt`
