...
6. `tzg-hourly-controller` 内的 Claude / DeepSeek wrapper 沿用上述主责限制；调度器必须先选中合法候选、取得单写入租约并给出任务授权范围，才可启动外部 CLI。无合法外部候选时不得预检或空转调用；wrapper 不等同于 Codex，不获得 Codex / ChatGPT5.5 主责或复审权限。
7. 外部责任方端到端调用 workspace guard、实施、运行最小充分验证、标记未审核、更新任务状态，并通过 `automation-finalize-commit.ps1` 创建路径限定的 `businessCommit`；随后只修改 `开发管理/AI合作沟通.txt`，登记真实业务 SHA、已验证、未验证和残留风险，再创建 `handoffCommit`，第二次不得重跑领域检查。外层 Codex只接收 `completed/needs_decision/blocked/failed`、两个 SHA 或恢复 session，不读业务 diff、不重验、不 stage、不 commit。外部责任方不得自审、扩大授权路径、并行派发或推送远端；WF3 保持 PAUSED。
...
| `开发管理/当前任务队列.txt` | `1` 执行工作计划与日常选题的短任务队列 |
| `开发管理/状态与建议维护规则.txt` | 状态事实、任务队列、长期建议的分层维护规则 |
| `开发管理/自动工作流规则.txt` | 每小时控制器的调度、选题、租约、恢复与失败关闭唯一规则源 |
| `开发管理/自动工作流状态.txt` | 自动化最近有效结果、队列校验和人工阻塞摘要 |
| `开发管理/总结规则.txt` | 用户说“总结”时必须执行的完整流程 |
| `开发管理/审核入口.txt` | 审核、复审与按需加载路由 |
| `开发管理/审核规则.txt` | 审核流程、审核归档与 AI 交接材料消费细则 |
| `开发管理/未通过审核清单.txt` | 当前未通过审核项与复审建议 |
| `开发管理/AI协作规则.txt` | 主责、交接、`1/2` 快捷流程与冲突处理 |
| `开发管理/AI合作沟通.txt` | DeepSeek ↔ Codex / ChatGPT5.5 当前交接队列；历史见 `开发管理/AI合作归档/` |
| `开发管理/DeepSeek工作提示词.txt` | DeepSeek 身份锚定与任务路由 |
...
纯 `1` / `2` 的身份确认、选题顺序、角色边界、任务标题和提交要求以 `开发管理/AI协作规则.txt` 为唯一完整规范。本文件保留的硬约束是：`1` 先从当前任务队列选择执行型任务，仅命中未审核、交接或复审路由时读取审核入口；`2` 先读审核入口并只做复审；DeepSeek 不得自审；任何非纯 `1` / `2` 的明确用户要求优先。
...
当本项目同时使用 ChatGPT5.5 与 DeepSeek V4 Pro 开发时，主责、交接和冲突处理以 `开发管理/AI协作规则.txt` 为准；用户明确指令优先，但仍必须遵守本文件与项目事实源规则。
