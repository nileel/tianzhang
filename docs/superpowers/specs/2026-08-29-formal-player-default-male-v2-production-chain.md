...
- 建模视图保留批准原画的右手单手剑指、右前臂抬至下胸前、左臂自然下垂和轻微自然重心，不得改成双臂下垂的对称建模人台；正面仍为严格正交，不继承原画的三分之四相机角度。
- 右手食指与中指并拢伸直，其余手指自然收拢；右肘靠近躯干，但右肘、前臂、手指、衣袖与躯干之间必须保留清楚可读的间隙，剑指不得遮挡脸、领口或主要服装结构。左手必须完整可见。
- 四个面板展示同一个冻结姿势，只改变正交观察方向；侧面和背面不得把右臂还原成下垂状态。若结果出现双臂下垂、姿势对称僵硬、剑指遮挡关键结构、手指融合或衣袖与身体粘连，视为姿势失败并停止。
