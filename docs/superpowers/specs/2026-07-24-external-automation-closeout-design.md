...
现有 Run ID 已经留下任务残留，人工按 `failed/external_task_residue` 如实记录并释放过期租约；随后才把本修复分支合并回主工作区。修复提交不追改该历史运行的结果。

## 测试

1. 扩展自动化工作流契约测试，要求控制器提示词明确包含外部完成后的 `RecordResult` 与 `Release`，并禁止把名称核验等同于终态收尾。
2. 扩展控制器契约 fixture，覆盖：
   - 合法外部双提交要求记录 success 并释放租约。
   - 非法终态要求记录 failed 后释放。
   - 存在新增未提交路径时要求保留现场与租约。
   - DeepSeek 身份规则缺失时静态检查失败。
