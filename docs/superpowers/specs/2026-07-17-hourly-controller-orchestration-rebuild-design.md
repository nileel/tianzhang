...
- 关联 Codex 任务：`019f701a-32b2-70b2-a13a-26565ea9a6fb`。
- 2026-07-16 14:53，先前 Codex 会话执行 `install-feishu-decision-bridge.ps1 -Action Install`，创建登录计划任务 `TianZhang-Feishu-Decision-Bridge`。
- 该任务当前以交互式身份直接启动长期存活的 `pwsh.exe`，再由 PowerShell 启动飞书桥 `node.exe`。任务虽设置 `Hidden=true` 且传入 `-WindowStyle Hidden`，登录时仍可能闪出或显示 PowerShell 控制台。
- 后台 `pwsh.exe` 本身是现有宿主结构的一部分；真正缺陷是可见控制台以及缺少不卸载私有配置即可安全启用、停用、启动和停止桥接的管理动作。
- 该问题不属于原编排层重建范围，现按负责人批准的方案 A 纳入：保持飞书登录自动连接，改为真正无窗口的 Windows 宿主启动，并提供显式生命周期管理。

## 3. 必须保留的组件

### 3.1 飞书通信链路

以下内容按现有通信行为保留，只允许为适配新控制器及修复已确认的登录可见窗口问题增加窄接口，不重写内部消息协议：
...
允许修改的唯一宿主边界是 `install-feishu-decision-bridge.ps1`、`start-feishu-decision-bridge.ps1` 及新增的无窗口启动器和对应直接测试。`tools/feishu-decision-bridge/` 内的卡片、事件、签名和回复协议不得因此改变。
...
计划任务不再直接执行 `pwsh.exe`。安装器解析系统 `wscript.exe` 的绝对路径，任务以 `//B //NoLogo` 调用项目内固定 VBS 启动器；启动器只接受经过安装器验证的 PowerShell 7 绝对路径和固定 `start-feishu-decision-bridge.ps1` 绝对路径，以窗口样式 `0` 启动 PowerShell 并等待其退出。路径含双引号、控制字符、非绝对路径或指向错误脚本时确定性拒绝。计划任务继续设置 `Hidden`、`IgnoreNew`、登录触发和 limited interactive principal，但登录期间不得出现或闪现控制台窗口。

后台进程树仍为 `wscript.exe -> pwsh.exe -> node.exe`；在任务管理器看到后台 `pwsh.exe` 不视为失败。用户可通过安装器的固定动作管理它：

- `Install`：创建或原位更新任务，启用并立即启动；私有配置先验证。
- `Start`：只启动已安装且已启用的任务；重复调用幂等。
- `Stop`：停止任务及其受验证进程树，但保持任务启用；下次登录仍会启动。
- `Disable`：先停止任务及其受验证进程树，再禁用登录启动；保留私有配置、配对和状态。
- `Enable`：启用已安装任务并立即启动；重复调用幂等。
- `Status`：在现有健康字段之外返回 `enabled` 与固定 `launchMode = WINDOWLESS_WSCRIPT`。
- `Uninstall`：删除任务，继续保留私有配置和状态。

停止进程只能通过计划任务停止语义和现有受验证的桥接进程匹配完成，不得按进程名批量结束任意 PowerShell 或 Node 进程。飞书被停止或禁用时，新控制器的决策发送动作返回 `feishu_unavailable`，保留待决策事实且不进入写入阶段；不得推断回复、自动改选或回退 Gmail。非决策任务不因桥接离线被错误标成已完成。
...
- `tools/start-feishu-decision-bridge-hidden.vbs`：无控制台窗口启动固定 PowerShell 宿主，不包含私有值。
- `tools/install-feishu-decision-bridge.ps1`：任务动作改为 `wscript.exe`，增加 `Start`、`Stop`、`Enable`、`Disable` 和扩展后的 `Status`。
- `tools/test-install-feishu-decision-bridge.ps1`：只用 fake scheduler 验证计划、生命周期动作、进程边界和脱敏，不在离线建设阶段触碰真实计划任务。
...
5. 先为登录可见窗口和生命周期动作补失败测试，再增加固定 VBS 无窗口启动器；该切片只运行安装器直接测试，不重复飞书 Node 全套测试。
...
5. 对安装器执行 `Plan`，确认任务目标为绝对 `wscript.exe`、参数为 `//B //NoLogo`、启动器不含私有值；此阶段不更新真实计划任务。
...
6. 在保持控制器暂停期间原位更新一次飞书计划任务，确认登录启动模式为 `WINDOWLESS_WSCRIPT`、桥接健康恢复且没有第二个实例；不要求为此重复 Node 协议回归。
...
- 无窗口宿主更新失败：停止并禁用飞书计划任务，保留私有配置和状态；修复后通过 `Enable` 恢复。不得回退到会显示控制台的直接 `pwsh.exe` 登录动作。
...
8. 飞书登录计划任务使用 `WINDOWLESS_WSCRIPT`，登录时无控制台显示或闪现；`Start`、`Stop`、`Enable`、`Disable`、`Status` 均保持私有状态并只影响受验证桥接进程树。
9. 桥接被停止或禁用时，待决策流程稳定返回 `feishu_unavailable`，不丢决定、不授权写入、不误报完成。
10. 故障注入后没有项目外泄、私有标识输出、残留租约、部分提交或自动重拍基线。
11. 合并后的 `master` 重跑全部相关测试并通过。
12. 首次 TQ-057 写入前，负责人审阅并明确批准 plan-only 工作清单。
