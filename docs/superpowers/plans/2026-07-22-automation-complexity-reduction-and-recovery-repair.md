...
- 自动化恢复链与薄控制器修复保留；两个生产 automation 继续保持 `PAUSED`。
- Task 6 曾恢复原 session 并生成 U-2_5D-01D 业务提交 `182e4aa`，但最终只读复核发现正式 `AdventureScene` 没有生产环境档案，迁移后会按失败关闭规则阻断相邻敌人战斗；后续 `3ed1cbd` 只修正移动负担，不能解决缺少生产输入的根因。
- 负责人选择回滚业务实现。`bb8050e` 已撤销 `182e4aa` 与 `3ed1cbd`，恢复正式冒险基线；U-2_5D-01D 改为阻塞于既有 C-ENV-PROFILE-01。Task 6 的“恢复原责任方并完成业务迁移”结果由本纠偏记录取代，不得把已回滚提交作为完成事实。
- 回滚过程中暴露的 SceneBuilder 重复创建同路径 Tile asset 问题由 `b31e152` 原位更新修复；该修复独立于空间契约，不改变自动化架构。

## 已确认的故障与停止条件

1. 计划启动时，`src/Assets/Tests/EditMode/SpatialQueryBoardTests.cs` 是自动责任方会话 `019f891d-a12a-7230-944a-0b9e1db14220` 在 2026-07-22 17:21 留下的半成品，不是人工文件；恢复处理期间不得删除、覆盖或当作普通冲突跳过。该证据已由 Task 6 消费，当前不再存在孤立文件，最终业务结果以上方执行纠偏为准。
