...
- 每张最终图严格为 `768×768` RGBA；Unity 导入合同为 `512 PPU`、`Sprite Alignment=Custom`、pivot `(0.5,0.18)`，即左下原点 `(384,138.24)` px。
- 2026-08-23 复核确认旧 QA 把图像顶原点的非透明包围盒 `y=96..623` 误写成左下原点。六张实际左下原点包围盒均为 `y=144..671`（含端点），可见高度仍为 `528 px`；四角 alpha 都是 `0`。共同 pivot 位于最低不透明像素下方 `5.76 px`、两足之间的画布中线，未以 Unity 方向私有缩放或位置偏移补偿；上方“实际完整提示词”保留生成时原文，不反写生成历史。
