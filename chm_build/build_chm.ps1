...
Write-Host "=== 1/3 生成 HTML ===" -ForegroundColor Cyan
Push-Location $root
...
const d=path.resolve('$safeRoot','html');
...
let hhp=fs.readFileSync(path.join(d,'project.hhp'),'utf8');
hhp=hhp.replace(/^Compiled file=.*$/m,'Compiled file=../tianzhang_design.chm');
fs.writeFileSync(path.join(d,'project.hhp'),hhp,'utf8');
...
try {
    ...
    Pop-Location
    if (-not (Test-Path "T:\tianzhang_design.chm")) { throw "HHC compilation failed" }
} finally {
    subst T: /d
}

# Rename using separate .mjs file (avoids PowerShell Unicode corruption)
node "$root\rename_chm.mjs"
if ($LASTEXITCODE -ne 0) { throw "CHM rename failed" }
