...
function New-Prompt {
  ...
  $routeInstruction = switch ($Route) {
    ...
    'QueueMaintenance' { '只做空队列维护，本轮不执行新增业务任务。先扫描各分线 backlog 中所有明确标为阻塞的任务；对阻塞描述中明确出现的稳定任务 ID，依次核对 开发管理/任务卡/<ID>.txt 与 开发管理/任务归档/<ID>.txt。backlog 中“阻塞”字样本身不能证明前置仍未完成：命名前置在活跃卡或完成归档中不存在时保持阻塞，不猜测完成状态；命名前置已完成但仍有其他真实条件时改写为实际剩余 blocker；全部前置已完成且现有事实足以形成完整任务卡时，同步 backlog、建立完整任务卡并按既有排序规则入队。完成全部阻塞项核对后仍没有合法候选，才允许返回 no_candidate。' }
  }
