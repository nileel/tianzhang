...
function Test-UnityPlayModeResult {
    param([string]$Path)

    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "Unity PlayMode result XML was not created: $Path"
    }
    ...
    try {
        ...
        throw "Unity PlayMode result XML could not be parsed: $Path. $($_.Exception.Message)"
    }

    $result = $document.DocumentElement
    if ($null -eq $result -or $result.Name -ne 'test-run') {
        throw "Unity PlayMode result XML must have a test-run root element."
    }
    ...
    if ($total -eq 0) {
        throw 'Unity PlayMode result XML reported zero executed tests.'
    }

    if ($total -ne ($passed + $failed + $inconclusive + $skipped)) {
        throw "Unity PlayMode result XML is incomplete: total=$total, passed=$passed, failed=$failed, inconclusive=$inconclusive, skipped=$skipped."
    }

    if ($failed -ne 0 -or $inconclusive -ne 0 -or $skipped -ne 0 -or $result.GetAttribute('result') -ne 'Passed') {
        throw "Unity PlayMode tests did not pass completely: passed=$passed, failed=$failed, inconclusive=$inconclusive, skipped=$skipped, result=$($result.GetAttribute('result'))."
    }
}
...
try {
    if ($ValidateResultOnly) {
        ...
        Test-UnityPlayModeResult -Path $ResultXmlPath
    }
    ...
    if (-not $ValidateResultOnly) {
        ...
        if ([string]::IsNullOrWhiteSpace($ResultXmlPath)) {
            $ResultXmlPath = Join-Path ([IO.Path]::GetTempPath()) 'tzg-playmode-test-results.xml'
        }
        ...
        $logPath = Join-Path ([IO.Path]::GetTempPath()) 'tzg-playmode-test-runner.log'
        $arguments = @(
            '-batchmode',
            '-nographics',
        ...
        if ($process.ExitCode -ne 0 -and -not $stoppedAfterValidResult) {
            throw "Unity PlayMode test process failed with exit code $($process.ExitCode). See $logPath"
        }
