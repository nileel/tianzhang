...
namespace TianZhang.Game.CharacterCreation
{
    public static class CharacterCreationCatalog
    {
        ...
        public static readonly SectRouteOption[] SectRoutes =
        {
            new SectRouteOption("route_taiyi", "太一道庭", "抱元守一经", new[] { "玄水咒", "沧浪击", "安神符" }),
            new SectRouteOption("route_yuqing", "玉清崖", "苦行剑典", new[] { "引雷诀", "苦行剑式", "剑罡护体" }),
            new SectRouteOption("route_hunyuan", "混元山", "含弘光大典", new[] { "铁骨功", "破阵冲锋", "玄甲铁壁" }),
            new SectRouteOption("route_taixu", "太虚观", "不真自虚法", new[] { "暗蚀", "幽冥引", "入梦诀" }),
            new SectRouteOption("route_sanxiu", "散修", "秋水游心经", new[] { "聚灵术", "川流劲", "灵光闪" }),
        };
        ...
        public static SectRouteOption FindSectRoute(string id) => SectRoutes.FirstOrDefault(item => item.Id == id);
    }
}
